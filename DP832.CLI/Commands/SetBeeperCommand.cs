...
{
    /// <summary>
    /// Enables or disables the instrument beeper via <c>:SYSTem:BEEPer</c>.
    /// Pass <c>--json</c> to receive the result as a JSON object.
    /// </summary>
    public sealed class SetBeeperCommand : Command<SetBeeperCommand.Settings>
    {
        ...
        public override int Execute(CommandContext context, Settings settings, CancellationToken cancellationToken)
        {
            ...
            using (var device = new DP832Device(settings.Address))
            {
                ...
                {
                    device.Connect();
                    device.SendCommand(":SYSTem:BEEPer " + (turnOn ? "ON" : "OFF"));
                    ...
                    {
                        AnsiConsole.MarkupLine("[green]Beeper " + (turnOn ? "enabled" : "disabled") + ".[/]");
                    }
                    return 0;
                }
                catch (Exception ex)
